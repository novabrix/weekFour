...
const calculateLateFee = function (ovDays) {
  const feeUnrefined = ovDays / 4;
  const fee = Math.round(feeUnrefined * 100) / 100;
...
const logCase = (name, num) => {
  return console.log(`Case #${num}: ${name}'s case is now logged.`);
};
...
const markAttendance = function (name, isPresent) {
  if (isPresent) console.log(`${name} is present.`);
  if (!isPresent) console.log(`${name} is absent.`);
};
...
function determineGrade(avScore) {
  if (avScore >= 90) {
    return "A";
  } else if (avScore < 90 && avScore > 79) {
    return "B";
  } else if (avScore < 80 && avScore > 69) {
    return "C";
  } else if (avScore < 70) {
    return "F";
  }
...
function generateReport(name, score1, score2, score3) {
  const avgUnrefined = calculateAverage(score1, score2, score3);
  const avg = Math.round(avgUnrefined * 100) / 100;
  const letter = determineGrade(avg);
  return console.log(`${name} - Average Score: ${avg}, Grade: ${letter}`);
}

generateReport("Amina", 85, 90, 78);
